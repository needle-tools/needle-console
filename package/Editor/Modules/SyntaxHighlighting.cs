...
namespace Needle.Demystify
{
	...
	internal static class SyntaxHighlighting
	{
		...
		public static void AddSyntaxHighlighting(string pattern, ref string line, bool trim = true)
		{
			...
			string Eval(Match m)
			{
				...
				for (var index = m.Groups.Count - 1; index >= 1; index--)
				{
					...
					var hasCol = CurrentTheme.TryGetValue(@group.Name, out var col);
					// fallback check if group name is color
					if (!hasCol && group.Name.Length == 6)
					{
						hasCol = ColorUtility.TryParseHtmlString("#" + group.Name, out _);
						col = group.Name;
					}
					if (hasCol)
					{
